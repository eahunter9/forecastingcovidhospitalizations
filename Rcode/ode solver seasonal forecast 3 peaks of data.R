...
Hosp_all_ode <- matrix(0,4000,592)
...
times <- seq(2, 592, by = 1)
out_test <- ode(yini, times, SEIR, pars, method ="bdf")

Hosp_per_day_ode <-  rep(0,592)
...
period <- c(rep("calibration",577),rep("projection",14))
time <- c(1:591)
...
real_data <- real_data[314:904,]
time <- c(1:591)

real_data$time  <- time

library(tidyverse)
g <- ggplot(sim_data, aes(x = time, y = measurement))+#, y = measurement, color = period, group =1)) +
